...
public class PageList extends PageSelect
{
	...
	public void init()
	{
		if (BYTE_ARRAY != null)
		{
			...
			this.char_2d_array[index++] = "CHUNK-LIST".toCharArray();
			...
			{
				i += 4+4;
				this.char_2d_array[index++] = ("" + (id++ + PAGE * 120)).toCharArray();
			}
	...
	public void enter()
	{
		...
		else
		{
			...
			else
			{
				...
				PageSelect ps = new PagePiece(ByteReader.getInt(BYTE_ARRAY, new_index), ByteReader.getInt(BYTE_ARRAY, new_index + 4));
...
//				STATE &= 255-1;
	...
	public void sendNet(byte b2)
	{
		...
		PageExtra.NET_BYTE_ARRAY[1] = SDaChunkList.ID;
