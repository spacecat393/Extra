...
import com.nali.list.gui.si.server.SSIELookTo;
...
public class PageSIELookTo extends PageSelect
{
	...
	public void init()
	{
		...
		{
			new BoxTextAll("SIE-LOOKTO".toCharArray()),
			new BoxTextAll("FLAG".toCharArray()),
			new BoxTextAll(("ON " + (byte)Math.signum(FLAG & SIELookTo.B_ON)).toCharArray()),
	...
	public void enter()
	{
		switch (this.select)
		{
			case 4:
				...
				this.sendNet(SSIELookTo.B_FETCH);
				break;
			case 5:
				this.back();
				break;
...
//				PageExtra.NET_BYTE_ARRAY[3 + 8] = (byte)(1 << this.select - 2);
	...
	public void sendNet(byte b2)
	{
		...
		PageExtra.NET_BYTE_ARRAY[1] = SSIELookTo.ID;
