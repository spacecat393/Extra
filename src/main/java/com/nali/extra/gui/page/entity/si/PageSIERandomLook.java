...
import com.nali.list.gui.si.server.SSIERandomLook;
...
public class PageSIERandomLook extends PageSelect
{
	...
	public void init()
	{
		...
		{
			new BoxTextAll("SIE-RANDOMLOOK".toCharArray()),
			new BoxTextAll("FLAG".toCharArray()),
			new BoxTextAll(("ON " + (byte)Math.signum(FLAG & SIERandomLook.B_ON)).toCharArray()),
	...
	public void enter()
	{
		switch (this.select)
		{
			case 4:
				...
				this.sendNet(SSIERandomLook.B_FETCH);
				break;
			case 5:
				this.back();
				break;
...
//				PageExtra.NET_BYTE_ARRAY[3 + 8] = (byte)(1 << this.select - 2);
				...
				this.sendNet(SSIERandomLook.B_SET);
		}
	}
	...
	public void sendNet(byte b2)
	{
		...
		PageExtra.NET_BYTE_ARRAY[1] = SSIERandomLook.ID;
