...
//		}
	...
	{
		PageExtra.NET_BYTE_ARRAY = new byte[1 + 1 + 1 + 4 + 4];
		PageExtra.NET_BYTE_ARRAY[0] = SPageDa.ID;
		PageExtra.NET_BYTE_ARRAY[1] = b1;
		PageExtra.NET_BYTE_ARRAY[2] = b2;
		if (b1 == SDaInv.ID)
		{
			ByteWriter.set(PageExtra.NET_BYTE_ARRAY, PageInv.PAGE, 3);
		}
		else
		{
			ByteWriter.set(PageExtra.NET_BYTE_ARRAY, PAGE, 3);
		}
		ByteWriter.set(PageExtra.NET_BYTE_ARRAY, ByteReader.getInt(PageInv.BYTE_ARRAY, 2 + PageInv.SELECT * 4), 3+4);
