...
import com.nali.list.gui.si.client.CSILeAttack;
...
//		ServerE s = ServerE.S_MAP.get(ByteReader.getLong(servermessage.data, 3));
		...
		SILeAttack sileattack = (SILeAttack)ms.getSI(entityplayermp, SILeAttack.ID);
		if (sileattack != null)
		{
			if (servermessage.data[2] == B_SET)
			{
				sileattack.state ^= servermessage.data[3+8];
			...
			if (servermessage.data[2] == B_FETCH)
			{
				...
				byte_array[1] = CSILeAttack.ID;
				byte_array[2] = sileattack.state;
