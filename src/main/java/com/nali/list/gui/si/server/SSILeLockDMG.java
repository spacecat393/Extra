...
import com.nali.list.gui.si.client.CSILeLockDMG;
...
//		ServerE s = ServerE.S_MAP.get(ByteReader.getLong(servermessage.data, 3));
		...
		SILeLockDMG silelockdmg = (SILeLockDMG)ms.getSI(entityplayermp, SILeLockDMG.ID);
		if (silelockdmg != null)
		{
			if (servermessage.data[2] == B_SET)
			{
				silelockdmg.flag ^= servermessage.data[3+8];
			...
			if (servermessage.data[2] == B_FETCH)
			{
				...
				byte_array[1] = CSILeLockDMG.ID;
				byte_array[2] = silelockdmg.flag;
